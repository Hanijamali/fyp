...
$st = $conn->prepare("
    SELECT s.file_path, s.student_id, b.student_id AS booking_student_id, tp.user_id AS tutor_user_id
...
if ($role === 'student' && $uid === (int) $row['student_id']) {
    ...
    $student_id = (int) $row['booking_student_id'];
    $ps->bind_param("ii", $uid, $student_id);
    $ps->execute();
    $ps->store_result();
